...
def need_to_search(file_path):
    ...
    for cell in asin_col:
        ...
        if g_cell is not None and g_cell != '-1':
            ...
            index+=1
            continue
        print('从第%d行开始，有%d个'% (index, size))
        cells = worksheet['B' + str(index): 'B' + str(index + size - 1)]
        for cell in cells:
            asin_list.append(cell[0].value)
        break
    # 写入元数据中
    meta_data = {'row_index':index,'asin_list':asin_list}
    return meta_data


# 写入本地，参数分别是目标文件、卖家数据data，data应该是字典，便于在写入前验证数据
def write_to_excel(target_file_path,data,row_index):
    ...
    for i in data:
        # print(i)
        print('表数据：%s，缓存数据:%s' %(worksheet['B' + str(row_index)].value, i['asin']))
        if worksheet['B' + str(row_index)].value != i['asin']:
            print('数据对不齐了，退出程序')
            break
        worksheet['F' + str(row_index)] = i['name']
        worksheet['G' + str(row_index)] = i['address']
        row_index += 1
    try:
...
def select_file(origin_dir):
    ...
    return origin_file
def prepare_target_file(origin_file_path,target_file_path):
    # 控制程序的进行
    work_flag = 1
